...
export const handleFirestoreError = (error: any, operation: string) => {
  ...
  if (error.code === 'permission-denied') {
    throw new Error('You do not have permission to perform this action. Please make sure you are logged in.');
...
export const safeFirestoreOperation = async <T>(
  ...
  try {
    ...
    throw error; // This will never be reached due to handleFirestoreError throwing
  }
};

// Wishlist operations
export const addToWishlist = async (userId: string, recipeId: string, recipeData: any) => {
...
export const subscribeToWishlist = (userId: string, callback: (items: any[]) => void, onError?: (error: Error) => void) => {
...
export const subscribeToUserRecipes = (userId: string, callback: (recipes: any[]) => void, onError?: (error: Error) => void) => {
  const recipesRef = collection(db, 'recipes');
  ...
  return onSnapshot(
    ...
    (error) => {
      ...
      if (onError) {
        onError(new Error('Failed to load your recipes. Please refresh the page.'));
      }
    }
  );
};
