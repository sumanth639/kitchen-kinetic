...
export function RecipeCard({ recipe, isFeatured = false }: RecipeCardProps & { isFeatured?: boolean }) {
  ...
  const cardContent = (
    <>
      ...
      {/* Enhanced Card Content - Adjusted padding and spacing for smaller screens */}
      <CardContent className=" cursor-pointer absolute bottom-0 left-0 right-0 h-[40%] sm:h-[50%] px-3 py-2 space-y-1 sm:space-y-2 bg-gradient-to-t from-card via-card/95 to-card/85 backdrop-blur-sm flex flex-col justify-center">
        {/* Title - Ensure tight line height on small screens */}
        <h3 className="font-bold capitalize text-lg leading-snug  group-hover:text-primary transition-colors duration-300 line-clamp-1 text-foreground">
          {recipe.title}
        </h3>
