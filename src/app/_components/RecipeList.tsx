...
import { ChevronLeft, ChevronRight, Soup, Search, Sparkles, SoupIcon } from 'lucide-react';
  ...
  return (
    <main className="container mx-auto px-4 sm:px-6 py-8 sm:py-12 max-w-[1440px]">
      <div className="min-h-[400px]">
        ...
        {!loading && !error && hasSearched && (
          <div className="mb-8 sm:mb-12">
            ...
            <p className="text-base sm:text-lg text-muted-foreground pl-12">
              <span className="font-semibold text-primary">{recipes.length}</span> recipes found for 
              <span className="font-medium text-foreground ml-1">"{searchTerm}"</span>
            </p>
          </div>
        )}
        ...
        {!loading && !error && !hasSearched && (
          <div className="mb-8 sm:mb-12">
            ...
            <p className="text-base sm:text-lg text-muted-foreground ">
              Handpicked popular dishes and custom recipes to inspire your culinary journey
            </p>
          </div>
        )}
        ...
        {/* Loading State */}
        {loading && (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-8 sm:gap-8 md:gap-8">
          ...
          <>
            {recipes.length > 0 ? (
              <>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-4 gap-8 sm:gap-10 md:gap-8 mb-8 sm:mb-12">
                ...
                {/* Enhanced Pagination - Only for Search Results */}
                  ...
                  <div className="flex flex-col sm:flex-row justify-center items-center gap-4 sm:gap-6 mt-12 sm:mt-16">
                    <div className="flex items-center gap-3">
                      <Button
                        ...
                        className="h-10 w-10 p-0 border-2 hover:border-primary hover:bg-primary hover:text-primary-foreground transition-all duration-300 disabled:opacity-40"
                        aria-label="Previous page"
                      >
                        <ChevronLeft className="h-4 w-4" />
                      </Button>
                      
                      <div className="flex items-center space-x-2 px-4 py-2 rounded-full bg-muted/50 dark:bg-muted/30 border border-border/50">
                    ...
                    <p className="text-xs sm:text-sm text-muted-foreground">
                      Showing {((currentPage - 1) * RECIPES_PER_PAGE) + 1}-{Math.min(currentPage * RECIPES_PER_PAGE, recipes.length)} of {recipes.length} recipes
                    </p>
                  </div>
                )}
              </>
                ...
                {hasSearched && (
                  <div className="mt-6 p-4 rounded-xl bg-muted/30 dark:bg-muted/20 border border-border/50 max-w-sm mx-auto">
                    <p className="text-sm text-muted-foreground">
                      <SoupIcon/> <span className="font-medium">Tip:</span> Try broader terms like "chicken", "pasta", or "dessert"
                    </p>
                  </div>
                )}
              </div>
