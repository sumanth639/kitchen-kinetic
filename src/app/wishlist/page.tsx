...
import { collection, onSnapshot, query, orderBy, doc, deleteDoc, CollectionReference } from 'firebase/firestore';
...
function WishlistCard({ recipe, onRemove }: { recipe: WishlistItem; onRemove: (id: string) => void }) {
    ...
    <Card className="bg-card text-card-foreground shadow-sm overflow-hidden group transition-all duration-300 hover:shadow-xl flex flex-col border rounded-lg">
      ...
      <CardHeader className="p-3 flex-grow">
        <CardTitle className="text-base font-semibold leading-snug">
           <Link href={`/recipes/${recipe.id}`} className="hover:underline">{recipe.title}</Link>
        </CardTitle>
      ...
      <CardFooter className="p-2 border-t">
          <Button variant="ghost" size="sm" className="w-full text-muted-foreground hover:text-destructive hover:bg-destructive/10" onClick={() => onRemove(recipe.id)}>
...
export default function WishlistPage() {
    ...
    useEffect(() => {
        ...
        const wishlistRef = collection(db, 'users', user.uid, 'wishlist') as CollectionReference<Omit<WishlistItem, 'id'>>;
    ...
    const handleRemoveFromWishlist = async (recipeId: string) => {
        ...
        try {
            ...
            toast({ title: "Error", description: "Failed to remove recipe from wishlist.", variant: "destructive"});
        }
    }
                ...
                <CardContent>
                    {wishlistLoading ? (
                        ...
                        <div className="text-center py-16 border-dashed border-2 rounded-lg flex flex-col items-center">
                            ...
                            <p className="text-muted-foreground text-sm">Save recipes you like to see them here.</p>
